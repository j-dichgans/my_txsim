...
def negative_marker_purity_cells(adata_sp: AnnData, adata_sc: AnnData, key: str='celltype', pipeline_output: bool=True):
    ...
    negative marker purity : float
       Increase in proportion of positive cells assigned in spatial data to pairs of genes-celltypes with no/very low expression in scRNAseq
    """   
    ...
    if len(celltypes) < 2:
        ...
        if pipeline_output==True:
            ...
            return negative_marker_purity, None, None           #wieso?
    ...
    lowvals_sc = ratio_celltype_sc.values[neg_marker_mask]        #flaches array?
    ...
    mean_sc_low_ratio = np.nanmean(lowvals_sc)    #wofür?
    ...
    if mean_sp_low_ratio > mean_sc_low_ratio:
        negative_marker_purity -= (mean_sp_low_ratio - mean_sc_low_ratio)        #wieso?
    
    if pipeline_output:
        return negative_marker_purity
    else:
        # Calculate per gene and per cell type purities
        purities = (ratio_celltype_sc - ratio_celltype_sp).cip(0,None)  #clip!?
...
def negative_marker_purity_reads(adata_sp: AnnData, adata_sc: AnnData, key: str='celltype', pipeline_output: bool=True):
    ...
    mean_ct_sc_rel = mean_celltype_sc.div(mean_celltype_sc.mean(axis=0),axis=1)       #wieso?
    ...
    if mean_sp_lowexp > mean_sc_lowexp:
        negative_marker_purity -= (mean_sp_lowexp - mean_sc_lowexp)         #wieso?
    ...
    if pipeline_output:
        ...
        # Calculate per gene and per cell type purities
        purities = (mean_ct_sp_norm - mean_ct_sc_norm).cip(0,None)          #clip?
        purities.values[~neg_marker_mask] = np.nan
        purities = purities.loc[~(purities.isnull().all(axis=1)), ~(purities.isnull().all(axis=0))]   
        #?alle Zeilen, in denen nicht alle Werte größer min_exp sind?
        purity_per_gene = purities.mean(axis=0, skipna=True)  #default is TRUE
        purity_per_celltype = purities.mean(axis=1, skipna=True)
