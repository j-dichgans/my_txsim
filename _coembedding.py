...
def knn_mixing(
    ...
    for ct in shared_cts:
        enough_cells = (adata.obs.loc[adata.obs[obs_key]==ct,"modality"].value_counts() > (ct_filter_factor * k)).all()     #all() unnötig?
        if enough_cells:
    ...
    mean_score = np.mean([v for _,v in scores.items() if v is not np.nan])      # wie funktioniert das hier mit np.nan?
